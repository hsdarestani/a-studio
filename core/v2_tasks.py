...
def _backend_requirements(project):
    ...
    if not features:
        return ""
    return "Requested backend capabilities: " + ", ".join(features) + ". Design the UI and product flow so these capabilities are represented clearly."
...
def import_and_provision_initial_project(self, project_id):
    ...
    try:
        ...
        prompt_parts = [
            f"Create the complete first version of this PWA for a {project.business_type} business.",
            f"Requirements: {project.description}",
            _backend_requirements(project),
        ]
        if imported_context:
            prompt_parts.append(
                "Use the following imported source as reference. Preserve useful product structure and business information, but do not copy secrets or executable code into the app specification:\n"
                + imported_context
            )
        result = propose_change(project, "\n\n".join(part for part in prompt_parts if part), [])
        ...
        with translation.override(_project_language(project)):
            Message.objects.create(
                ...
                role="assistant",
                status="failed",
                content=_("The first build could not be completed safely. The source import or build log needs review."),
